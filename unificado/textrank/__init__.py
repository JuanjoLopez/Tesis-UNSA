...
def text_rank(text, summary_length=100, clean_sentences=False, language='english'):
    """Return a paragraph formatted summary of the source text.
...
def summarize_all():
    ...
    for article in articles:
        ...
        text_ranks.append(text_rank(text))
...
def generar_puntuaciones():
    resultado=[]
    textranks=summarize_all()
    for i in range(len(textranks)):
        puntaje=[]
        diccionario=textranks[i] # grafo de un texto
        for oracion,tr_puntuacion in diccionario.items():
            puntaje.append(i)
            puntaje.append(tr_puntuacion)
